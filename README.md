...
JavaScript," Section 3.5) — no Next.js, no TypeScript, no framework beyond what's actually named.
See [`docs/claude-code-build-guide.md`](docs/claude-code-build-guide.md) for the full reasoning
behind each tooling decision.
...
├── design/        DESIGN.md (the design system) + design/screens/ (Stitch exports per screen)
└── docs/          Build guides and process documentation from this project's development
```

Each of `backend/` and `frontend/` has its own `CLAUDE.md` documenting entity schemas, API
contracts, known gaps, and lessons learned during development — worth reading before making
changes to either.
...
registration, QR scanning, verification) and UAT are manual, documented separately in `docs/`.
