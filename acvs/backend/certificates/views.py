...
class CertificateDetailView(APIView):
    ...
    def patch(self, request, certificate_id: str):
        # 1. Decode and sanitize the incoming URL parameter to handle spaces (%20)
        clean_id = urllib.parse.unquote(certificate_id).strip()
        ...
        # 2. Extract and validate the new certificate ID if provided in the payload
        new_cert_id = request.data.get("certificate_id")
        if new_cert_id is not None:
            new_cert_id = str(new_cert_id).strip()
            if not new_cert_id:
                raise ValidationError({"certificate_id": "Certificate ID cannot be blank."})
            
            # Check if another certificate already uses this ID
            if Certificate.objects.filter(certificate_id=new_cert_id).exclude(pk=cert.pk).exists():
                raise ValidationError({"certificate_id": "A certificate with this number already exists."})
            
            cert.certificate_id = new_cert_id

        # 3. Handle other fields using the serializer
        ser = CertificateEditSerializer(cert, data=request.data, partial=True)
        ser.is_valid(raise_exception=True)
        ser.save()

        # Explicitly save the certificate instance in case certificate_id was modified directly
        cert.save()

        changed_fields = sorted(list(ser.validated_data.keys()) + (["certificate_id"] if new_cert_id else []))
